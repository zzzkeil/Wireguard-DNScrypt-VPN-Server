...
echo -e " ${GRAYB}#######################################################################################################################################${ENDCOLOR}"
echo -e " ${GRAYB}#${ENDCOLOR} ${GREEN}test for hennefun with pihole @ home ${ENDCOLOR}"
echo -e " ${GRAYB}#######################################################################################################################################${ENDCOLOR}"
echo -e " ${GRAYB}#######################################################################################################################################${ENDCOLOR}"
...
echo "--------------------------------------------------------------------------------------------------------"
echo "--------------------------------------------------------------------------------------------------------"
read -p "clients ipv4 network: " -e -i 12.12 wg0networkv4
echo "--------------------------------------------------------------------------------------------------------"
...
echo "--------------------------------------------------------------------------------------------------------"
echo "--------------------------------------------------------------------------------------------------------"
read -p "clients ipv6 network: " -e -i 12:12:12 wg0networkv6
echo "--------------------------------------------------------------------------------------------------------"
echo "--------------------------------------------------------------------------------------------------------"
...
# maybe wrong....
firewall-cmd --zone=trusted --add-forward-port=port=53:proto=tcp:toport=53:toaddr=10.$wg0networkv4.2
firewall-cmd --zone=trusted --add-forward-port=port=53:proto=udp:toport=53:toaddr=10.$wg0networkv4.2
...
touch /etc/wireguard/keys/clientpihole
chmod 600 /etc/wireguard/keys/clientpihole
wg genkey > /etc/wireguard/keys/clientpihole
wg pubkey < /etc/wireguard/keys/clientpihole > /etc/wireguard/keys/clientpihole.pub
...
[Peer]
PublicKey = PKPI
AllowedIPs = 10.$wg0networkv4.2/32, fd42:$wg0networkv6::2/128
...
sed -i "s@PKPI@$(cat /etc/wireguard/keys/clientpihole.pub)@" /etc/wireguard/wg0.conf
...
PrivateKey = CK01
DNS = 10.$wg0networkv4.2, fd42:$wg0networkv6::2
$wg0mtu
[Peer]
...
echo "[Interface]
Address = 10.$wg0networkv4.2/32
Address = fd42:$wg0networkv6::2/128
PrivateKey = CKPI
DNS = 10.$wg0networkv4.2, fd42:$wg0networkv6::2
$wg0mtu
[Peer]
